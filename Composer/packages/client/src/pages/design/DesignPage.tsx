...
const DesignPage: React.FC<RouteComponentProps<{ dialogId: string; projectId: string; skillId?: string }>> = (
  ...
  useEffect(() => {
    ...
    PluginAPI.publish.startProvision = async (config) => {
      ...
      const fullConfig = { ...myconfig, name: props.dialogId, type: 'azurePublish' };
      ...
      await provisionToTarget(fullConfig, config.type, projectId, arm, graph, undefined);
    };
  }, [provision]);
    ...
    <div css={contentWrapper} role="main">
      ...
      <button
        onClick={() => {
          ...
          window.parent?.postMessage({ type: 'provisionInfo', publishInfo: myconfig }, '*');
        }}
      >
        Publish
      </button>
      {provision && (
        <Dialog
          ...
          onDismiss={() => setProvision(false)}
        >
          <Fragment>
            <div style={{ marginBottom: '16px' }}>{'Provision Bot Resources'}</div>
            <div css={azurePublishSurface}>
              <PluginHost
                bundleId="azurePublish"
                pluginName="azurePublish"
                pluginType="publish"
                projectId={projectId}
              />
            </div>
